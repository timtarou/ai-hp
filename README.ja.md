...
- **Claude Code の `/usage`、Codex の `/status`**：そのツールでいまログインしている 1 アカウントだけを表示します。ai-hp はこのマシンの全アカウントを、両ツールをまたいで次のリセット順に並べるので、いまどれを使うべきかが分かります
- **メニューバーのアプリ（CodexBar など）**：macOS のメニューバーに常に出したいならそちらが便利です。ai-hp はコマンドとスキルなので、macOS・Linux・WSL のどのターミナルでも（SSH 先でも）動き、エージェントが読めて、手元の `claude` と `codex` の CLI とだけやり取りします
