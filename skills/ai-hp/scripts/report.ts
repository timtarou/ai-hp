...
import { formatDateTime, formatFullDateTime, formatRemaining } from "./time.ts";
import type { Limit, Snapshot } from "./types.ts";

const PROVIDER_NAME = { claude: "Claude", codex: "Codex" } as const;
const PROVIDER_ORDER = { claude: 0, codex: 1 } as const;
...
export type ReportInput = {
  fresh: Snapshot[];
  /** 今回は取得できず、前回値を表示するアカウント */
  cached: Snapshot[];
  /** 表示しなかったフォルダと理由（未ログイン等） */
  excluded: string[];
  problems: string[];
  /** 表のあとに出す「ひとこと」1 行（commentary.ts） */
  comment?: string;
  now: Date;
  host: string;
  timeZone: string;
 ...
 */
export function indicator(remainingPct: number): string {
  let filled = Math.round(remainingPct / (100 / BAR_CELLS));
  if (remainingPct > 0) filled = Math.max(filled, 1);
  if (remainingPct < 100) filled = Math.min(filled, BAR_CELLS - 1);
...
function resetText(limit: Limit, now: Date, timeZone: string): string {
  if (!limit.resetsAt) return t("notStarted");
  if (limit.resetsAt <= now) return t("resetDone", { time: formatDateTime(limit.resetsAt, timeZone) });
...
function shortTitle(title: string): string | undefined {
  if (title === "Full reset" || title === "reset") return undefined;
...
function creditsCell(s: Snapshot, now: Date, timeZone: string): string {
  ...
  if (count === 0) return note ? `0${paren(note)}` : "0";
...
function accountCell(s: Snapshot, stale: boolean, timeZone: string): string {
 ...
 */
function sortKey(s: Snapshot, now: Date): [number, number, string] {
