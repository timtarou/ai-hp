...
type Parts = Record<"year" | "month" | "day" | "hour" | "minute" | "weekday", string>;
...
function parts(d: Date, timeZone: string): Parts {
  ...
  return out as Parts; // formatToParts は上の 6 項目を必ず返す
}
...
/** 2026-09-25 14:10 */
export function formatFullDateTime(d: Date, timeZone: string): string {
  const p = parts(d, timeZone);
...
export function formatRemaining(from: Date, to: Date): string {
  ...
  const m = minutes % 60;
  if (d > 0) return t("inDaysHours", { d, h });
  if (h > 0) return t("inHoursMinutes", { h, m });
