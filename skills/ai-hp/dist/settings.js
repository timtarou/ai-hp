...
import { isLang, langFromLocale } from "./i18n.js";
...
export function configDir() {
    if (process.env.XDG_CONFIG_HOME)
        return path.join(process.env.XDG_CONFIG_HOME, "ai-hp");
    if (process.platform === "win32" && process.env.APPDATA)
        return path.join(process.env.APPDATA, "ai-hp");
...
export function cacheDir() {
    if (process.platform === "darwin")
        return path.join(homedir(), "Library", "Caches", "ai-hp");
    if (process.platform === "win32" && process.env.LOCALAPPDATA)
        return path.join(process.env.LOCALAPPDATA, "ai-hp");
...
export async function loadSettings(argv, env = process.env) {
    ...
    const tz = [env.AI_HP_TZ, str(file.timeZone)].find((v) => !!v && isValidTimeZone(v));
    return {
        lang,
...
export function debugLog(label, value) {
    if (debugEnabled)
        console.error(`[ai-hp debug] ${label}: ${JSON.stringify(value, null, 2)}`);
}
