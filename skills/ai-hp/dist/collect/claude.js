...
async function readAccount(configDir) {
    let raw;
    try {
...
export function parseClaudeUsage(usage) {
    ...
    const push = (l, limit) => {
        ...
        if (l && used !== undefined)
            limits.push({ ...limit, usedPercent: used, resetsAt: dateOrUndefined(l.resets_at) });
    };
    if (entries.length > 0) {
        push(entries.find((l) => l.kind === "weekly_all"), { kind: "weekly" });
        for (const l of entries.filter((e) => e.kind === "weekly_scoped"))
            push(l, { kind: "scoped", scope: scopeName(l) });
        push(entries.find((l) => l.kind === "session"), { kind: "short", windowHours: 5 });
    }
    else {
        ...
        for (const [key, limit] of [
            ...
            if (used !== undefined)
                limits.push({ ...limit, usedPercent: used, resetsAt: dateOrUndefined(w?.resets_at) });
        }
    }
