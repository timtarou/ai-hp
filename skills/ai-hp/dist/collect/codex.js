...
import { exchangeJsonLines } from "../jsonl-process.js";
...
function toWindow(v) {
    ...
    if (used === undefined || windowMins === undefined)
        return undefined;
 ...
 */
export function pickWindows(rateLimits) {
    const windows = [toWindow(rateLimits?.primary), toWindow(rateLimits?.secondary)].filter((w) => w !== undefined);
    return {
...
export function parseCodexRateLimits(result) {
    ...
    const limits = [];
    if (weekly)
        limits.push({ kind: "weekly", usedPercent: weekly.usedPercent, resetsAt: weekly.resetsAt });
    // 既定以外の枠（モデル別など）があれば別枠として並べる
    for (const [limitId, other] of Object.entries(obj(result?.rateLimitsByLimitId) ?? {})) {
        if (limitId === str(main?.limitId))
            continue;
        const w = pickWindows(obj(other)).weekly;
        const scope = str(obj(other)?.limitName) ?? limitId;
        if (w)
            limits.push({ kind: "scoped", scope, usedPercent: w.usedPercent, resetsAt: w.resetsAt });
    }
    if (shortTerm) {
        limits.push({
            kind: "short",
