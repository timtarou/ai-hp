...
import { remainingPercent } from "./report.js";
...
function labeler(snapshots) {
    const base = (s) => `${PROVIDER_NAME[s.provider]} ${s.email.split("@")[0]}`;
    const counts = new Map();
    for (const s of snapshots)
        counts.set(base(s), (counts.get(base(s)) ?? 0) + 1);
    return (s) => {
        const b = base(s);
        if ((counts.get(b) ?? 0) < 2 || !s.plan)
            return b;
        return `${b}/${s.plan}`;
    };
}
function usableCredits(s, now) {
