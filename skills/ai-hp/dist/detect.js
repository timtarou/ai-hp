 ...
 */
export async function detectSources(home = homedir()) {
    const names = (await readdir(home, { withFileTypes: true }))
        .filter((e) => e.isDirectory() || e.isSymbolicLink())
        .map((e) => e.name)
        .sort();
    const has = (dir, file) => existsSync(path.join(home, dir, file));
    const claudeDirs = fromEnv("AI_HP_CLAUDE_DIRS") ??
        names
            .filter((n) => n === ".claude" || (/^\.claude-[\w-]+$/.test(n) && has(n, ".claude.json")))
            .map((n) => path.join(home, n));
    const codexHomes = fromEnv("AI_HP_CODEX_HOMES") ??
        names
